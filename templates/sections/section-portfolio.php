        ...
        <div id="sec-portfolio-wrap" class="js-masonry grid-masonry-wrap <?php businessx_anim_classes(); ?>" data-masonry-options='{ "columnWidth": ".sec-portfolio-grid-sizer", "gutter": ".sec-portfolio-gutter-sizer", "percentPosition": true, "itemSelector": ".grid-col" }'>
			...
			if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post();
            	...
            	<p class="ta-center msg-info">
					<?php printf( __( 'Ready to publish your first project? <a href="%1$s">Get started here</a>.', 'businessx-extensions' ), esc_url( admin_url( 'post-new.php?post_type=jetpack-portfolio' ) ) ); ?>
                </p>
            <?php 
				endif; // Query 
				else : // If Jetpack or Portfolio module isn't enabled
			?>
            	<p class="ta-center msg-info"><?php _e( 'You need <b>Jetpack - Portfolio</b> module enabled to use this section. Projects will appear here once you activate the plugin.', 'businessx-extensions' ); ?></p>
            <?php
				endif; // JetPack check	
			?>
