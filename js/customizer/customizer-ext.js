...
jQuery( document ).ready( function( $ ) {
	...
	var bx_frontPageSections = $( '#accordion-panel-businessx_panel__sections > .accordion-sub-container' ),
	...
	$( '#accordion-panel-businessx_panel__sections > .accordion-sub-container > .panel-meta' ).after( bx_DragDropMsg );
	...
	wp.customize.section.each( function ( section ) {
		$.each( bx_allsections, function( index, value ) {
			...
			var currentSectionID	 	= '#accordion-section-businessx_section__' + value,
			...
			$(document).on('click', '#accordion-section-sidebar-widgets-section-' + value + ' .add-new-widget', function(event) {
			...
			if( section.id == bxSectionValue ) { // Just in the actual section
				...
				if( checkHiddenOption.is( ':checked' ) ) {
					currentSectionSlctID.addClass( hiddenSectionClass );
					$( addNewWidgetSlct.selector ).attr( 'disabled', true );
				} else {
					currentSectionSlctID.removeClass( hiddenSectionClass );
			...
			if( section.id == bxSectionSidebar ) { // Just in the selected sidebar
				...
				var accordionSec = '#accordion-section-';
	...
	wp.customize.control.each( function ( control ) { 
		$.each( bx_allsections, function( index, value ) {
			...
			// Move item to sidebar to make focus available
			var goBackField = value + '_go_back';
			if( control.id == goBackField ) {
				wp.customize.control( goBackField ).section( 'sidebar-widgets-section-' + value );
				wp.customize.control( goBackField ).priority( '-2' );
				$( '#customize-control-' + goBackField ).hide().find( 'input' ).attr( 'disabled', true );
			}
